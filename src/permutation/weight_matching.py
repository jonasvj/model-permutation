"""
Code taken from:
https://github.com/themrzmaster/git-re-basin-pytorch/blob/main/utils/weight_matching.py
The code is a PyTorch re-implementation of the source code from the paper "Git
Re-Basin: Merging Models modulo Permutation Symmetries" (https://arxiv.org/abs/2209.04836).
"""
...
def weight_matching(ps: PermutationSpec, params_a, params_b, max_iter=100, init_perm=None, verbose=True):
    ...
    for iteration in range(max_iter):
        progress = False
        for p_ix in torch.randperm(len(perm_names), device=device):
            p = perm_names[p_ix]
            n = perm_sizes[p]
