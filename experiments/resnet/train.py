...
def main(cfg):
    ...
    num_channels = dm.data_train[0][0].shape[0]
    num_batches = len(dm.train_dataloader())

    # Initialize model
    model = eval(cfg.model['class'])(
        num_channels=num_channels, num_train=num_train, **cfg.model.hparams
    )
    model.to(device)
    ...
    torch.save(
        {
            ...
            'model_hparams': {'num_channels': num_channels, 'num_train': num_train, **cfg.model.hparams},
